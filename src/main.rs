...
fn new_shader(name: &str) -> String {
    format!("#if defined(VERTEX) || __VERSION__ > 100 || defined(GL_FRAGMENT_PRECISION_HIGH)
#define MY_HIGHP_OR_MEDIUMP highp
#else
#define MY_HIGHP_OR_MEDIUMP mediump
#endif

extern MY_HIGHP_OR_MEDIUMP vec2 {};
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {{
    ...
    vec4 final_color = vec4(1.0, 1.0, 1.0, clamp({}.x, 1.0, 1.0));
